...
class MenuModuleSerializer(serializers.ModelSerializer):
    class Meta:
        model = MenuModule
        fields = ('id', 'menu_module_id', u'name', )

class PrivilegeSerializer(serializers.ModelSerializer):
    class Meta:
        model = Privilege
        fields = ('id', 'privilege_id', 'name')
...
class RptGoalrecordSerializer(serializers.ModelSerializer):
    class Meta:
        model = RptGoalrecord
        fields = ('matchid', 'score', 'score_a', 'score_b', 'goal', 'hit', 'land', 'active', 'game')
...
class NewVideoCommentSerializer(serializers.ModelSerializer):
    content = serializers.CharField(max_length=800, allow_blank=True)
    class Meta:
        model = VideoComment
        fields = ('id', 'video_name', 'parent_id', 'content', 'user_id')

class VideoCommentSerializer(serializers.ModelSerializer):
    class Meta:
        model = VideoComment
        fields = ('id', 'video_name', 'parent_id', 'content', 'user_id', 'username', 'profile_photo')


class NewFavoriteFolderSerializer(serializers.ModelSerializer):
    url = serializers.CharField(max_length=1000, allow_blank=True)
    class Meta:
        model = FavoriteFolder
        fields = ('id', 'name', 'parent_id', 'url', 'record_type', 'user_id')

class FavoriteFolderSerializer(serializers.ModelSerializer):
    record_type_name = serializers.CharField(source='get_record_type_display')
    class Meta:
        model = FavoriteFolder
        fields = ('id', 'name', 'parent_id', 'url', 'record_type', 'record_type_name')

class VideoProcInfoSerializer(serializers.ModelSerializer):
    init_name = serializers.CharField(source='get_init_display')
    state_name = serializers.CharField(source='get_state_display')
    class Meta:
        model = VideoProcInfo
        fields = ('id', 'match_pkid', 'match_id', 'match_name', 'task_id', 'tb_name', 'init', 'init_name', 'state', 'state_name',
            'task_is_failured', 'error_code', 'error_info', 'created_at', 'updated_at')
